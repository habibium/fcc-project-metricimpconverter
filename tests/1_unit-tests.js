...
suite("Unit Tests", function () {
  ...
  suite("Test convertHandler.getReturnUnit()", () => {
    ...
    test("convertHandler should correctly convert gal to L.", () => {
      ...
      assert.closeTo(convertHandler.convert(10, "gal"), 37.8541, 0.00001);
    });

    test("convertHandler should correctly convert L to gal.", () => {
      assert.closeTo(convertHandler.convert(1, "L"), 0.2641721769, 0.00001);
      assert.closeTo(convertHandler.convert(10, "L"), 2.6417217686, 0.00001);
    });

    test("convertHandler should correctly convert mi to km.", () => {
      assert.closeTo(convertHandler.convert(1, "mi"), 1.60934, 0.00001);
      assert.closeTo(convertHandler.convert(10, "mi"), 16.0934, 0.00001);
    });

    test("convertHandler should correctly convert km to mi.", () => {
      assert.closeTo(convertHandler.convert(1, "km"), 0.6213711922, 0.00001);
      assert.closeTo(convertHandler.convert(10, "km"), 6.2137119224, 0.00001);
    });

    test("convertHandler should correctly convert lbs to kg.", () => {
      assert.closeTo(convertHandler.convert(1, "lbs"), 0.453592, 0.00001);
      assert.closeTo(convertHandler.convert(10, "lbs"), 4.53592, 0.00001);
    });

    test("convertHandler should correctly convert kg to lbs.", () => {
      assert.closeTo(convertHandler.convert(1, "kg"), 2.20462, 0.00001);
      assert.closeTo(convertHandler.convert(10, "kg"), 22.0462, 0.00001);
    });
  });
});
